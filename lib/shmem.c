...
struct shm_ptr* shmem_new(const char* name, size_t size)
{
	...
	__ptr->addr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, __fd, 0);
...
struct shm_ptr* shmem_open(const char* name)
{
	...
	__ptr->addr = mmap(__ptr->addr, __ptr->size, PROT_READ | PROT_WRITE, MAP_SHARED, __fd, 0);
	if(__ptr->addr == MAP_FAILED)
		goto error_end;
...
void shmem_close(struct shm_ptr* ptr)
{
	...
	if(munmap(ptr->addr - sizeof(struct shm_ptr), ptr->size) < 0)
		perror("munmap error.");
